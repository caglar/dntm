...
def safe_grad(cost, params, known_grads=None):
    ...
    grads = OrderedDict({param: grad \
                            for param, grad in safe_izip(params, grad_list)})
    return grads
...
def sym(w):
    return w.dot(sqrtm(inv(w.T.dot(w))))


def sample_weights_orth(sizeX, sparsity, scale=1.0, rng=None, use_sym=False):
    sizeX = int(sizeX)
    sizeY = sizeX
