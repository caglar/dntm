...
from memnet.mainloop import FBaBIMainLoop, NTMToyMainLoop
from memnet.nmodel import NTMModel
from memnet.fbABIdataiterator_singleq import FBbABIDataIteratorSingleQ
from memnet.ntm_tasks import copy_data
from memnet.ntm_tasks.associative_recall import AssociativeRecall
...
def search_model_adam(state, channel, reload_model=False):
    ...
    def get_inps(use_mask=True, vgen=None, debug=False, use_qmask=False, output_map=None):
    ...
    max_len = (item_len + 1) * (max_nitems + 2)
    ...
    # For RNN controller:
    learn_h0 = state.get('learn_h0', True)
    ...
    correlation_ws = state.get('correlation_ws', 2e-4)
    use_qmask = True

    n_layers = state.get('n_layers', 1)

    anticorr = state.get('anticorr', None)
    prfx = ("dntm_ar_learn_h0_%(learn_h0)d_as_%(address_size)d_rs_%(renormalization_scale)f_hids_%(n_hids)d_bsize_%(batch_size)d"
            "_std_%(std)f_mem_nel_%(mem_nel)d_mem_size_%(mem_size)f_lr_%(lr)f_uff_%(use_ff_controller)d") % locals()
    save_path = state.get("save_path", ".")
    prfx = save_path + prfx

    tdata_gen = AssociativeRecall(batch_size,
                                  max_nitems=max_nitems,
                                  item_len=item_len,
                                  inp_size=inp_size,
                                  use_qmask=use_qmask,
                                  rng=rng,
    ...
    vdata_gen = AssociativeRecall(batch_size,
                                  max_nitems=max_nitems,
                                  item_len=item_len,
                                  inp_size=inp_size,
                                  use_qmask=use_qmask,
    ...
    tst_data_gen = AssociativeRecall(batch_size,
                                     max_nitems=max_nitems,
                                     item_len=item_len,
                                     inp_size=inp_size,
                                     use_qmask=use_qmask,
    ...
    inps = get_inps(vgen=vdata_gen, debug=debug, output_map=True, use_qmask=use_qmask)
